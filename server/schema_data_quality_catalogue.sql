-- ═══════════════════════════════════════════════════════════════════════════
-- Morocco With You — Catalogue data quality: diagnostics, normalization, constraints
--
-- Deploy (pick one):
--   • Supabase CLI: server/migrations/20260329120000_data_quality_catalogue.sql
--   • SQL Editor: run sections B–D below (same as migration), then optionally
--     server/schema_data_quality_constraints_optional.sql for CHECKs
--
-- Apply city rewrites (after reviewing v_city_spelling_candidates):
--   server/schema_data_quality_normalize_apply.sql
-- ═══════════════════════════════════════════════════════════════════════════

-- ── A) Canonical city mapping (reference — extend as you discover variants) ─
--
-- Canonical (display) → common raw variants seen in spreadsheets / imports
--
--   Fès          ← fez, fes, fès, Fès el-Bali (use city=Fès, keep detail in location)
--   Marrakech    ← marrakesh, marraquech, rak, "menara", "medina marrakech" (whole-field only)
--   Casablanca   ← casa, casablanca, csa
--   Rabat        ← rabat
--   Tangier      ← tangier, tanger, tànger
--   Chefchaouen  ← chaouen, chefchaouen, xauen
--   Essaouira    ← essaouira, essouira, mogador
--   Merzouga     ← merzouga, erg chebbi, sahara (when used as sole city token)
--   Agadir       ← agadir
--   Ouarzazate   ← ouarzazate, ouarzazat
--   Imlil        ← imlil (Atlas gateway — keep distinct from Marrakech)
--
-- Note: `location` often holds neighborhoods ("Guéliz, Marrakech") — we only normalize
-- when the whole trimmed value matches a known alias, or you run manual fixes.

-- ── B) Normalization helper (single text → canonical city label or NULL) ─────
...
-- ── C) Diagnostic view — rows missing MVP-critical content ─────────────────
...
-- Quick counts: SELECT * FROM v_catalogue_data_quality_summary;

-- ── D) Find suspicious city / location values (manual review list) ──────────
...
-- ── E) Normalization DML — use schema_data_quality_normalize_apply.sql ─────

-- ── F) Optional CHECK constraints — see schema_data_quality_constraints_optional.sql
