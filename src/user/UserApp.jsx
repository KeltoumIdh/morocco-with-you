...
export default function UserApp() {
  ...
  const [generatedItinerary, setGeneratedItinerary] = useState(() => readItineraryFromSession());
  ...
  useEffect(() => {
    const h = (e) => { if (e.key === "Escape") { setChatOpen(false); setAuthGate(null); } };
  ...
  useEffect(() => {
    ...
    try {
      sessionStorage.setItem(ITINERARY_SESSION_KEY, JSON.stringify(generatedItinerary));
    } catch {
      /* quota */
    }
  }, [generatedItinerary]);
  ...
  useEffect(() => {
    ...
    itinerariesApi
      ...
      .then((res) => {
        ...
        const latest = list[0];
        if (!cancelled && latest?.content?.days?.length) setGeneratedItinerary(latest);
      })
      .catch(() => {})
      .finally(() => {
  ...
  const renderPage = () => {
    switch (page) {
      ...
      case "group-trip":
        return <GroupTripDetailPage trip={selectedGroupTrip} setPage={navigateTo} />;
      case "event":    return <EventRequestPage setPage={navigateTo} />;
      case "ai":       return <AIPage setPage={navigateTo} setGeneratedItinerary={setGeneratedItinerary} />;
      case "itinerary":
        return (
          <ItineraryPage
            setPage={navigateTo}
      ...
      case "chat":     return <ChatPage onOpenCatalogueItem={handleOpenCatalogueItem} />;
      case "profile":  return <ProfilePage setPage={navigateTo} />;
      case "login":    return <LoginPage setPage={setPage} />;
      case "register": return <RegisterPage setPage={setPage} />;
      case "booking-success":
        return <BookingSuccessPage setPage={navigateTo} />;
      default:         return <HomePage setPage={navigateTo} setGeneratedItinerary={setGeneratedItinerary} setSelectedExperience={setSelectedExperience} />;
    }
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-screen" style={{ background: "var(--parchment)" }}>
        <div className="text-center">
          <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-white font-bold text-sm mx-auto mb-4"
            style={{ background: "linear-gradient(135deg,var(--terracotta),var(--gold))" }}>MW</div>
          <p className="text-sm" style={{ color: "var(--smoke)" }}>Loading…</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ background: "var(--parchment)", color: "var(--ink)", minHeight: "100vh", fontFamily: "'DM Sans',sans-serif" }}>
      ...
      {/* ── Mobile layout ── */}
      <div className={`md:hidden ${isChatPage ? "flex flex-col" : ""}`}
        style={isChatPage ? { height: "100dvh", overflow: "hidden" } : {}}>
        {!isChatPage && !isAuthPage && (
          <header className="glass fixed top-0 left-0 right-0 z-30 px-4 py-3 flex items-center gap-3"
            ...
            <div className="ml-auto flex items-center gap-2">
              {isAuthenticated && isAdmin && <AdminDashboardLink variant="inline" />}
              {!isAuthenticated && (
        ...
        <main className={isChatPage ? "flex-1 flex flex-col overflow-hidden" : ""}>
          {renderPage()}
        </main>
      ...
      {/* ── FAB ── */}
      {showFab && (
        <button onClick={handleFabClick} aria-label="Open AI chat" className="ai-float"
          style={{ position: "fixed", bottom: 24, right: 24, zIndex: 50, width: 56, height: 56, borderRadius: 18, background: "linear-gradient(135deg,var(--terracotta),var(--deep))", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 8px 32px rgba(192,101,74,.4)" }}>
          <ChatIcon />
          {unread > 0 && !chatOpen && (
            <span className="badge-ping" style={{ position: "absolute", top: -3, right: -3, width: 8, height: 8, borderRadius: "50%", background: "#E53E3E", border: "2px solid var(--parchment)" }} />
          )}
        </button>
      )}
      ...
      {chatOpen && !isMobile && (
        <>
          <div onClick={() => setChatOpen(false)}
            style={{ position: "fixed", inset: 0, zIndex: 48, background: "rgba(26,20,16,.18)", backdropFilter: "blur(2px)", animation: "fadeIn .25s ease forwards" }} />
          <div ref={panelRef} className="slide-up"
            style={{ position: "fixed", bottom: 0, right: 24, zIndex: 49, width: 380, height: 580, borderRadius: "24px 24px 0 0", background: "rgba(250,246,238,.92)", backdropFilter: "blur(20px) saturate(160%)", WebkitBackdropFilter: "blur(20px) saturate(160%)", border: "1px solid rgba(212,168,83,.2)", borderBottom: "none", boxShadow: "0 -8px 48px rgba(26,20,16,.2)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px 12px", borderBottom: "1px solid rgba(232,196,160,.5)", flexShrink: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ width: 34, height: 34, borderRadius: 12, background: "linear-gradient(135deg,var(--terracotta),var(--gold))", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 11, fontWeight: 700 }}>AI</div>
                <div>
                  <p style={{ fontSize: 14, fontWeight: 600, color: "var(--ink)", margin: 0 }}>Morocco AI Guide</p>
                  <p style={{ fontSize: 11, color: "var(--sage)", margin: 0 }}>● Online · Instant replies</p>
                </div>
              </div>
              <button onClick={() => setChatOpen(false)}
                style={{ background: "var(--light-clay)", border: "none", borderRadius: 10, width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "var(--smoke)", flexShrink: 0 }}>
                <CloseIcon />
              </button>
            </div>
      ...
      {authGate && (
        <AuthGateModal
          ...
          onLogin={() => { setAuthGate(null); setPage("login"); }}
          onRegister={() => { setAuthGate(null); setPage("register"); }}
